...
public class HashMapEx {
	...
	public static void main(String[] args) {
		...
		//전체 객체 순회 2 : forEadch의 콜백함수에 key value 인수로 전달함
		System.out.println("--------------------");
		map.forEach((key, value)->{
			System.out.println(key + ":" + value);
		});
		
		//전체 객체 순회 3 : entrySet() 메소드 활용 -> key=value 형태의 entry를 Set 타입으로 반환 메서드
		System.out.println("--------------------");
		for(Entry<String, Integer> entrySet : map.entrySet()) {// 제네릭 타입 결정 필요
