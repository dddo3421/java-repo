...
public class CatMain {
	...
	public static void main(String[] args) {
		...
		//Dog 클래스 참조변수 선언
		//ap는 타입은 Animal 이지만 참조 객체는 Dog 타입의 객체를 참조중임(자동타입변환)
		//아래와 같이 대입하면 타입 불일치 에러 발생 -> 단, ap는 Dog 객체를 참조 중이고 Dog-> Animal로 자동형변환이 적용된 상태임
		//일시적으로 강제형변환 가능
		Dog d = (Dog)ap;
		d.dogMethod();
		...
		//부모타입 객체가 모두 자식타입으로 강제 형변환이 가능한 건 아님 -> 자식타입에 근거 한 객체여야함
		Animal aa = new Animal();//aa 참조변수는 참조객체의 타입이 Animal임
		//Dog dd = (Dog)aa; // 런타임에 형변환 발생 -> 실행시에 예외 발생 cannot be cast to class
