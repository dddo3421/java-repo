...
public class TypeCasting {
	...
	public static void main(String[] args) {
		...
		char chrVal = '가';
		intVal = chrVal; //2byte 크기의 값이 4byte 변수에 저장
		System.out.println(intVal); // '가'의 유니코드 값 44032
		
		long lngVal = 258960; //8byte
		float fVal = lngVal; //4byte - 값의 표현 범위는 float이 더 넓음 자동형변환 가능 
		System.out.println(fVal);
		
		// 강제 타입 변환 - 데이터 손실이 발생함(주의해서 변환되어야 함)
		//intVal = fVal; // float형의 표현범위가 int형보다 크므로 큰 범위의 표현이 작은 범위로 -> 대입 불가능
		fVal = 25.369847f;
		intVal = (int)fVal; //(int)리터럴 또는 변수 -> int형으로 강제 타입변환 발생하게 됨
		System.out.println(intVal);
		...
		byte byteValue = (byte)intVal; //정수를 정수타입에 저장 - 큰타입을 작은타입에 
 		...
 		//  정밀도 손실 발생 가능 
 		// float의 가수 23bit보다 큰 int값을 float으로 변활할 때 손실 발생 => int를 실수로 변경 double형으로 변환
