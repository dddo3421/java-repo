...
public class Operators {
	...
	public static void main(String[] args) {
		...
		//short result =-a; // short type 변수 a에 부호연산 -를 적용하면 반환 타입은 int(큼)-> short(작음)에 저장 타입미스매치
		...
		y1 = x1++; // ++가 대입연산 이후로 밀림 x1의 값을 먼저 y1에 대입(y=10)시키고 x1을 1 증가 11이 됨
		System.out.println(y1);
		y1 = ++x1; // 11인 x1을 먼저 증가시키고(x1 = 12) y1에 대입 => y1은 12가 대입
		System.out.println(y1);
		
		x=1;
		int y=1;
		
		System.out.println(++x); //x를 증가시켜서 출력
		System.out.println(y++); //출력하고 y 증가
		
		x=1;
		y=1;
		System.out.println(++x+10); // x를 증가시켜서 10을 더함 : 12
		System.out.println(y++ +10); // y를 10하고 더한 후에 y값을 1증가시킴
		System.out.println(y);  // 위에서 ++로 증가된 y가 출력: 2
		...
		int z2 = x2 * y2; // 조 -> 결과가 int형으로 반환되어 오버플로우 발생
		System.out.println(z2); // -727379968 오버플로우 발생하면서 1조 뒤에서부터 저장하다가 모자라면 저장하지 않음
		
		long x3 = 1000000; // 백만
		int y3 = 1000000; // 백만
		
		long z3 = x3 * y3; // 조 -> x3가 long 형이라  long x int면 long이 되어버림 
		System.out.println(z3); // 
		//피연산자가 단위가 크면 결과 저장할 변수의 타입을 크게 잡는다 long타입 사용 
		
		// / 연산 또는 %연산 일 경우
		// 정수 0 또는 실수 0.0으로 나누는 경우 - 예외발생하거나 Infinity 출력
		//System.out.println(10/0); // 예외 발생 - 아래코드 실행하지 않고 프로그램 출력
		System.out.println(10/0.0); //Infinity 
		
		// %연산
		// 정수 0 또는 실수 0.0으로 나누는 경우 - 예외발생하거나 NaN 출력
		//System.out.println(10%0); // / by zero , 프로그램 종료됨
		...
		System.out.println(city != gu); //city의 값과 gu의 값이 다른지의 여부 판단하지 않음 - 변수가 참조하는 주소가 같은지 판단
		...
		//문자열 객체 메소드 equals 활용 값을 비교하는 기능
